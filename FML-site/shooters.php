...
<body>
	...
	$result=mysqli_query($conn,"SELECT Name,OwnerNum,Owner1,Owner2,Owner3,Goal FROM current WHERE Goal>".$num1." ORDER BY Goal DESC");
	...
	<table>
		<tr><th>球员</th><th>球队</th><th>进球</th></tr>");
		$result=mysqli_query($conn,"SELECT Name,OwnerNum,Owner1,Owner2,Owner3,resGoal FROM current WHERE resGoal>".$num2." ORDER BY resGoal DESC");
		while($row=mysqli_fetch_assoc($result)){
			echo("<tr><td>".$row['Name']."</td><td>");
			if($row['OwnerNum']==3){
